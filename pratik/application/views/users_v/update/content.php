        ...
        <div class="widget">
            <div class="widget-body">
                <form action="<?php echo base_url("users/update/$item->id"); ?>" method="post">
                    ...
                    <div class="form-group">
                        <label for="role">Kullanici Rolu</label>
                        <select name="role" id="role" style="width: 100%" class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
                            <?php foreach ($users_roles as $user_role) { ?>
                                <option value="<?php echo $user_role->id; ?>"<?php echo($user_role->id==$item->role) ? 'selected' : ''; ?>><?php echo $user_role->title ?></option>
                            <?php } ?>
                        </select>
                        <?php if(isset($form_error)){ ?>
